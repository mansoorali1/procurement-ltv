...
def build_feature_vector(inputs: dict, feature_meta: dict) -> pd.DataFrame:
    """
    Convert raw user inputs into the 44-feature vector the model expects.
    Uses global fallback values from feature_metadata for seller/category
    encoded features since we don't have live seller history.
    """
    f = inputs
    ...
    row = {
        ...
        "customer_state_avg_variance"   : feature_meta.get(
            "global_cat_avg", -11.24
        ),
        "seller_state_avg_variance"     : feature_meta.get(
            "global_cat_avg", -11.24
        ),
        "seller_avg_variance"           : feature_meta.get(
            "global_seller_avg", -11.24
        ),
        "seller_std_variance"           : feature_meta.get(
            "global_seller_std", 9.66
        ),
        "seller_order_count"            : 5,
        "seller_late_rate"              : feature_meta.get(
            "global_seller_late", 0.08
        ),
        "customer_order_count"          : 1,
        "is_repeat_customer"            : 0,
        "category_avg_variance"         : feature_meta.get(
            "global_cat_avg", -11.24
        ),
    }
...
def render(model, feature_meta, validation_results):
    st.header("📦 Procurement Lead Time Variance Predictor")
    st.markdown(
    ...
    if st.button("🔮 Predict Delivery Variance", type="primary", use_container_width=True):
        ...
        with st.expander("📌 Model Notes"):
            ...
            - **Test MAE:** {validation_results['test_metrics']['mae']:.2f} days
            - **Test R²:** {validation_results['test_metrics']['r2']:.4f}
            - **Directional Accuracy:** {validation_results['test_metrics']['dir_acc']:.1f}%
            - **Note:** Seller-specific history not available for new orders.
              Global averages used for seller features.
            """)